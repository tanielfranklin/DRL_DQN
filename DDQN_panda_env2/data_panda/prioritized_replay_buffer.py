...
class PrioritizedReplayBuffer:
    ...
    def load_buffer(self,folder):
        ...
        with open('runs/'+folder+'/priorities.pickle', 'rb') as handle:
            self.priorities=pickle.load(handle)
        print("Replay Buffer and priorities loaded")
        
    def save_buffer(self,folder):
        with open(folder+'/buffer.pickle', 'wb') as handle:
            pickle.dump(self.buffer, handle, protocol=pickle.HIGHEST_PROTOCOL)
           
        with open(folder+'/priorities.pickle', 'wb') as handle:
            pickle.dump(self.priorities, handle, protocol=pickle.HIGHEST_PROTOCOL)
        print("Replay Buffer and priorities Saved")
